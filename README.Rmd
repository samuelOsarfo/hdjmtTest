...
- **Usage**:  
  ...
  `app_orth_fast(y, x, chosen_M, COV.S=NULL, k = 1)` — _fast version using Rcpp_
- **Arguments**:
  - `y`: Outcome vector.
  - `x`: Exposure vector.
...
- **Usage**:  
  ...
  `get_active_med.fast(y, x, M, COV.S=NULL, pval.adjust='HDMT', variant=1, d=NULL, r=1, k=1)` — _fast version using `app_orth_fast()` and supports multiple null estimation variants_
...
- **Arguments**:
  ...
  - `variant`: (Only for `get_active_med.fast`) Specifies which null estimation function to use if `pval.adjust = 'HDMT'`. Use `1` (default) for modified, `2` for original HIMA, and `3` for HDMT.
