#' Get Active Mediators ::Modified Null Estimation Function
#'
...
#' active_mediators_index <- get_active_med.mod(y, x, M)
#'
...
#' active_mediators_index <- get_active_med.mod(y, x, M, pval.adjust='bonferroni')
#'
#' #Print the indexes of active mediators
#'print(active_mediators_index)
get_active_med.mod <- function(y, x, M, COV.S=NULL, pval.adjust='HDMT', d=NULL, r=1,  k=1){
  ...
  #get index for active mediators in chosen mediators
     ...
     active_index <- js_test_mod(chosen_ind, alp_all$pval, ao_obj$pval, method=pval.adjust)
